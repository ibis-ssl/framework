...
- **停止は2経路あり、片方は惰走。実機と構造が一致している。** 実機 G474 は
  `sys->stop_flag || stop_emergency || !is_vision_available ||
  elapsed_time_ms_since_last_vision > 500` で `omniStopAll()` に入り
  （`state_func.c:314`）、4輪の電圧を 0 にするだけ（`omni_wheel.c:62`）。CAN フレームは
  duty の float 4 バイトのみでブレーキビットを持たない（`actuator.c:12`）。つまり
...
- **feedback の速度は指令が届いたときしか更新されない。** 速度（byte 52/56）は
  ...
  よる停止では指令自体は届き続けるので、速度は正しく 0 まで減衰する。
